    ...
    public long maximumProfit(int[] prices, int k) {
        ...
        for (int day = 1; day < n; day++) {
            ...
            for (int trans = k; trans > 0; trans--) {
                ...
                dp[trans].maxProfit = 
                    Math.max(dp[trans].maxProfit, 
                    Math.max(dp[trans].buyHold + 
                    currPrice, dp[trans].sellHold - currPrice));
                dp[trans].buyHold = 
                    Math.max(dp[trans].buyHold, prevProfit - currPrice);
                dp[trans].sellHold = 
                    Math.max(dp[trans].sellHold, prevProfit + currPrice);
            }
        }
